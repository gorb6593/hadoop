...
public class Test {
	...
	public static void main(String[] args) {
		...
		//=> 정수 값은 객체가 할당된 주소를 가지고 내부에서 생성된다.
		//=> hashcode메소드를 이용하면 이 정수 값이 리턴된다.
		...
		key1.setYear("abc1987");
		key2.setYear("ccc1987");
		
		//문자여레서 호출하는 해시코드는 Object클래스의 hashcode메소드가 오버라이딩되어
		//주소를 가지고 해시코드값을 계산하지 않고 문자열을 이용해서 정수 값을 만든다.
		//따라서 문자열이 같으면 해시코드값이 같다.
		System.out.println("=====Customkey객체의 year데이터의 해쉬코드=====");
